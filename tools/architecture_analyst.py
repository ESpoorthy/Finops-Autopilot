...
class ArchitectureAnalyst:
    """
    Analyzes infrastructure metrics (CPU/Memory utilization, node count) from Cloud Monitoring (LIVE MODE) or demo data (DEMO MODE).
    """
    ...
    def get_cluster_performance(self, cluster_name: str = "prod-core-cluster") -> PerformanceMetric:
        ...
        try:
            ...
            # In live mode query kubernetes.io/container/cpu/utilization
            # Fall back to demo metrics if live monitoring metrics return empty/unconfigured
            return self._get_demo_performance_metrics(cluster_name)
        except Exception:
            return self._get_demo_performance_metrics(cluster_name)
